...
export default function ReceiptsLedgerReportPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['receipts-ledger', range, type, selectedSchoolId],
    queryFn: async () => {
      const params = new URLSearchParams(range);
      if (type) params.append('type', type);
      if (selectedSchoolId) params.append('schoolId', selectedSchoolId);
      const res = await api.get(`/reports/receipts-ledger?${params}`);
      return res.data;
    },
  });
  ...
  const handleExportPdf = async () => {
    if (contentRef.current) {
      await exportElementToPdf(contentRef.current, `รายงานรับชำระ-${range.startDate}.pdf`);
    }
  };
  ...
  const handleExportCsv = () => {
    ...
    data.receipts.forEach((r: any) =>
      rows.push([r.receiptNo, fmtDate(r.date), typeLabels[r.type] || r.type, r.school || '-', r.amount]),
    );
    exportToCsv(`รายงานรับชำระ-${range.startDate}`, rows);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-display font-bold text-slate-900">รายงานรับชำระ</h1>
          <p className="text-slate-500 mt-1">รายการใบเสร็จรับเงินในช่วงเวลาที่กำหนด</p>
        </div>
        ...
        <div ref={contentRef} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="card p-4">
              ...
              <p className="text-xl font-bold text-emerald-700">{fmt(data.summary.totalAmount)}</p>
            </div>
          </div>
          ...
          <div className="card overflow-hidden">
            <div className="px-4 py-3 bg-slate-50 border-b border-slate-200">
              <h3 className="font-semibold text-slate-900">รายการใบเสร็จ ({data.receipts.length})</h3>
            </div>
