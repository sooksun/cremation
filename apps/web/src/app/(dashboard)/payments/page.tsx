...
export default function PaymentsPage() {
  ...
  const { data: payments, isLoading } = useQuery<PaymentItem[]>({
    queryKey: ['payments', selectedSchoolId, selectedYear],
    queryFn: async () => {
  ...
  const { data: summary } = useQuery({
    queryKey: ['payments-summary', selectedSchoolId, selectedYear],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (selectedSchoolId) params.append('schoolId', selectedSchoolId);
      params.append('startDate', `${selectedYear}-01-01`);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showSuccess('บันทึกใบสำคัญจ่ายสำเร็จ');
      closeModal();
    },
    onError: (error: any) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            บันทึกรายจ่ายของสมาคม ปี พ.ศ. {selectedYear + 543}
          </p>
        </div>
      ...
      <div className="card overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-20 text-slate-500">
            ...
            <p className="text-lg font-medium">ยังไม่มีใบสำคัญจ่าย</p>
          </div>
        ) : (
            ...
            <table className="table">
              ...
              <tbody>
                {payments?.map((payment) => (
                  <tr key={payment.id}>
                    <td className="font-mono text-sm">{payment.voucherNo}</td>
                    <td>
                    ...
                    <td>
                      <span className="badge-warning">{typeLabels[payment.type]}</span>
                    </td>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold">บันทึกใบสำคัญจ่าย</h2>
